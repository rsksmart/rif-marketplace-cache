...
import { getStakesForAccount } from './models/stake.model'
...
export class AvgBillingPriceService extends Service {
  ...
  async get (): Promise<{ min: number, max: number }> {
    ...
    const [{ avgPrice: min }] = await sequelize.query(getAvgMinMaxBillingPriceQuery(-1), {
      type: QueryTypes.SELECT,
      raw: true
    })
    const [{ avgPrice: max }] = await sequelize.query(getAvgMinMaxBillingPriceQuery(1), {
      type: QueryTypes.SELECT,
      raw: true
    })
    return { min, max }
